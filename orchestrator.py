...
from utils.email_notifier import get_notifier, EmailNotifier
...
def setup_logging(log_file: Path = None, log_level: int = logging.INFO) -> Path:
    """
    ...
    Returns:
        Path to log file (creates timestamped file if none provided)
    """
...
class MQCMDBOrchestrator:
    ...
    def __init__(self, enable_email: bool = True, log_file: Path = None):
        """
        ...
        # Setup logging
        self.log_file = setup_logging(log_file)

        # Initialize email notifier
        self.email_notifier = get_notifier() if enable_email else None
        ...
        try:
            ...
            if Config.ENABLE_OUTPUT_CLEANUP:
                ...
                if cleanup_results['total_deleted'] > 0:
                    ...
                    logger.info(f"Cleaned up {cleanup_results['total_deleted']} old files")
            ...
            logger.info(f"Loading MQ CMDB data from {Config.INPUT_JSON}")
            ...
            logger.info("Processing MQ Manager relationships")
            ...
            if baseline_file.exists():
                ...
                logger.info("No baseline found - creating first baseline")
            ...
            logger.info("Generating hierarchical topology diagram")
            ...
            logger.info("Generating application diagrams")
            ...
            if count > 0:
                ...
                logger.info(f"Generated {count} application diagrams")
                if not pdf_generated:
            ...
            logger.info("Generating individual MQ manager diagrams")
            ...
            if individual_count > 0:
                ...
                logger.info(f"Generated {individual_count} individual MQ manager diagrams")
                if not pdf_generated:
                    safe_print("⚠ GraphViz required for PDF generation")
            ...
            logger.info("Generating smart filtered views")
            try:
                ...
                if filtered_count > 0:
                    ...
                    logger.info(f"Generated {filtered_count} filtered view diagrams")
            ...
            try:
                ...
                if gateway_analytics['summary']['total_gateways'] > 0:
                    ...
                    logger.info("No gateways found in data")
            except Exception as e:
            ...
            logger.info("Generating multi-format exports")
            try:
            ...
            logger.info("Generating Enterprise Architecture documentation")
            try:
                ...
                logger.error(f"EA documentation generation failed: {e}")
    ...
    def _send_notification(self, success: bool, error_message: str = None):
        ...
        if not self.email_notifier or not self.email_notifier.is_enabled:
            logger.debug("Email notifications disabled, skipping")
            return

        logger.info("Sending email notification...")

        # Add log file and errors to summary
        summary = self._summary_stats.copy()
        summary["Log File"] = str(self.log_file)
        ...
        result = self.email_notifier.send_pipeline_notification(
            success=success,
            summary=summary,
        ...
        if result:
            ...
            logger.warning(f"Failed to send email notification: {self.email_notifier.errors}")
            safe_print(f"⚠ Failed to send email notification")
            for err in self.email_notifier.errors:
                safe_print(f"  - {err}")


def main():
    """Entry point for the MQ CMDB orchestrator."""
    # Check for environment variable to enable email
    enable_email = os.environ.get("EMAIL_ENABLED", "").lower() in ("true", "1", "yes")

    orchestrator = MQCMDBOrchestrator(enable_email=enable_email)
