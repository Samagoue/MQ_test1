...
def _group_by_country(
    ...
    """
    # Collect associations per country from Pass 1 records
    ...
    # All countries represented by either Pass 1 records or a known channel
...
@PluginRegistry.register(order=1.5)
class AssetAssociationStep(Processor):
    """Cross-country asset association using pattern substitution.

    Reads patterns.json and countryCode.json from Config.INPUT_DIR.
    Uses context.raw_data as the asset source (populated by DataIngestionStep).
    Falls back to reading Config.INPUT_JSON directly if raw_data is not loaded.
    Writes grouped output to Config.ASSET_ASSOCIATIONS_JSON.
    """
    ...
    def execute(self, context: PipelineContext) -> None:
        ...
        if not country_path.exists():
            logger.info(f"  Skipped: {country_path.name} not found in input/")
            return

        # ── Source asset list ─────────────────────────────────────────────
        raw_data: Optional[List[Dict]] = context.raw_data
        if not raw_data:
            if not config.INPUT_JSON.exists():
                logger.info(f"  Skipped: {config.INPUT_JSON.name} not found")
                return
        ...
        # ── Pass 2: build country → channel map ───────────────────────────
        country_channel_map = _scan_builtin_patterns(raw_data, cty)
        logger.info(
            f"  Pass 2 (built-in): {len(country_channel_map)} channel(s) identified"
        )

        # ── Pass 1: JSON pattern matching (parallel) ──────────────────────
        flat_records: List[Dict] = []
        total_skipped = 0
        ...
        if patterns_path.exists():
            ...
            logger.info(f"  Pass 1 skipped: {patterns_path.name} not found in input/")

        # ── Group by country (one channel, many associations) ─────────────
