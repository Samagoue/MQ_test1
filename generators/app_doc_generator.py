...
class ApplicationDocGenerator:
    ...
    def generate_app_page(self, app_name: str) -> Optional[str]:
        ...
        lines.extend([
            ...
            "",
            "||MQ Manager||Local||Remote||Alias||Total||Gateway||",
        ])
        for mgr_name in sorted(app_info['mqmanagers']):
            ...
            lines.append(f"|{mgr_name}|{ql:,}|{qr:,}|{qa:,}|{ql+qr+qa:,}|{gw}|")
        lines.append("")
        ...
        # Close documentation tab
        lines.extend([
            "{card}",
        ])

        # ---- Card: Integration Diagram tab ----
        lines.extend([
            "{card:label=Integration Diagram}",
            "",
