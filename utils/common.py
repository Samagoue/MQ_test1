...
Centralizes shared helpers: string sanitization, color manipulation,
hierarchical data iteration, and console formatting.
"""
...
def iter_mqmanagers(enriched_data: Dict) -> Generator[Tuple[str, Dict], None, None]:
    """
    Iterate over all MQ managers in the enriched hierarchical data.

    Flattens the 5-level hierarchy (Org -> Dept -> BizOwner -> App -> MQMgr)
    into a simple (name, data) stream.

    Args:
        enriched_data: Enriched hierarchical MQ CMDB data

    Yields:
        (mqmanager_name, mqmanager_data) tuples
    """
