...
# ── Asset Association publishing ──────────────────────────────────────────────

def publish_asset_association_doc(
    file_path,
    timestamp: Optional[str] = None,
    version_comment: Optional[str] = None,
) -> bool:
    """
    Publish the Asset Associations Confluence markup file as a child page.

    Parent page resolution order (confluence_config.json):
        1. asset_association_parent_page_id   — used directly if set (no API call)
        2. asset_association_parent_page_name — resolved via get_page_by_title()
        3. page_id                            — top-level page as last resort

    Args:
        file_path:       Path to the generated Asset_Associations_*.txt markup file.
        timestamp:       Pipeline run timestamp string (used in version comment).
        version_comment: Override version comment.
    ...
    PAGE_TITLE = "Cross-Country Asset Associations"

    try:
        client, config = _get_client()

        if not config.get("publish_asset_associations", False):
            logger.info("  Confluence: asset association publishing disabled in config")
            return False

        file_path = Path(file_path)
        if not file_path.exists():
            logger.warning(f"⚠ Asset associations file not found: {file_path}")
            return False

        # ── Resolve parent page ID ────────────────────────────────────────
        parent_id = config.get("asset_association_parent_page_id", "").strip()

        if not parent_id:
            parent_name = config.get("asset_association_parent_page_name", "").strip()
            if parent_name:
                space_key = config.get("space_key", "").strip()
                if not space_key:
                    # Derive space_key from the top-level page
                    top_id = config.get("page_id", "").strip()
                    if top_id:
                        try:
                            top_page = client.get_page(top_id, expand="space")
                            space_key = top_page.get("space", {}).get("key", "")
                            logger.info(f"  Resolved space_key '{space_key}' from page {top_id}")
                        except Exception as e:
                            logger.warning(f"  Could not resolve space_key: {e}")

                if space_key:
                    try:
                        found = client.get_page_by_title(space_key, parent_name)
                        if found:
                            parent_id = found.get("id", "")
                            logger.info(f"  Resolved parent '{parent_name}' → page ID {parent_id}")
                        else:
                            logger.warning(
                                f"⚠ Parent page '{parent_name}' not found in space '{space_key}'"
                            )
                    except Exception as e:
                        logger.warning(f"⚠ Parent page lookup failed for '{parent_name}': {e}")
                else:
                    logger.warning("⚠ Cannot resolve parent by name — space_key not available")

        if not parent_id:
            parent_id = config.get("page_id", "").strip()

        if not parent_id:
            logger.warning("⚠ Confluence: no parent page resolved for asset associations — skipping")
            return False

        # ── Resolve space_key from parent (needed to create new child page) ──
        space_key = config.get("space_key", "").strip()
        if not space_key:
            try:
                parent_page = client.get_page(parent_id, expand="space")
                ...
                logger.warning(f"  Could not resolve space_key from parent {parent_id}: {e}")

        comment = version_comment or f"Auto-updated by MQ CMDB Pipeline — {timestamp or ''}"

        # ── Create or update child page ───────────────────────────────────
        existing_children = client.get_child_pages(parent_id) or []
        child_by_title = {child["title"]: child["id"] for child in existing_children}

        if PAGE_TITLE in child_by_title:
            page_id = child_by_title[PAGE_TITLE]
            client.update_page_from_file(
                page_id=page_id,
                title=PAGE_TITLE,
            ...
            logger.info(f"✓ Confluence: updated '{PAGE_TITLE}' (ID {page_id})")
        else:
            ...
            new_id = result.get("id", "?")
            logger.info(f"✓ Confluence: created '{PAGE_TITLE}' (ID {new_id})")

        return True

    except ConfluenceError as e:
        logger.error(f"Confluence API error publishing asset associations: {e}")
        return False
    except Exception as e:
        logger.error(f"Failed to publish asset associations: {e}")
        return False
